...
interface BodyUpdateProfile extends Omit<User, '_id' | 'roles' | 'createdAt' | 'updatedAt' | '__v' | 'email'> {
  password?: string
  newPassword?: string
}
...
const userService = {
  ...
  uploadAvatar(body: FormData) {
    return httpRequest.post<SuccessResponseApi<string>>('user/upload-avatar', body, {
      headers: {
